...
#define PATH_MAX 1024
    ...
    EMUC_OPEN_DEVICE_SCT    EMUCOpenDeviceSCT;
...
static void get_ini_setting (void)
{
    ...
    memset(dest, 0, sizeof(dest));
        ...
        if(readlink(path, dest, PATH_MAX) >= 0)
        {
            len = strlen(dest);
...
static void demo_emuc (void)
{
    ...
    rtn = EMUCOpenDeviceSCT(ini_item.com_port);
    if(rtn)
    {
        #if defined(__linux__) || defined(__QNX__)
...
static void load_dll (void)
{
    ...
    else
    {
        ...
        EMUCOpenDeviceSCT   = (EMUC_OPEN_DEVICE_SCT)   GetProcAddress(hDLL, "EMUCOpenDeviceSCT");
        ...
        if(!EMUCOpenDevice || !EMUCCloseDevice || !EMUCShowVer || !EMUCResetCAN  || !EMUCClearFilter  ||
           ...
           !EMUCReceiveNonblock || !EMUCEnableSendQueue || !EMUCGetBusError || !EMUCSetRecvBlock || !EMUCOpenDeviceSCT)
        {
